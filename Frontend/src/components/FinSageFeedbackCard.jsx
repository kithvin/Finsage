...
export default function FinSageFeedbackCard() {
  ...
  return (
    <section className="my-10 sm:my-12 px-3 sm:px-6 lg:px-0">
      <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl border border-white/10 shadow-sm">
        {/* Background */}
        <div
          className="absolute inset-0"
          style={{
        ...
        {/* Glow */}
        <div className="pointer-events-none absolute -top-20 sm:-top-24 -right-20 sm:-right-24 h-56 sm:h-72 w-56 sm:w-72 rounded-full bg-[#EF8354]/25 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-20 sm:-bottom-28 -left-20 sm:-left-28 h-56 sm:h-72 w-56 sm:w-72 rounded-full bg-white/10 blur-3xl" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-stretch">
            ...
            <div className="flex flex-col justify-between">
              <div>
                <div className="inline-flex items-center gap-2 rounded-2xl bg-white/10 border border-white/10 px-3 py-2">
                  <MessageSquareText className="h-4 w-4 text-[#EF8354]" />
                  <span className="text-[10px] sm:text-xs font-semibold tracking-widest text-white/80">
                    SHARE YOUR FEEDBACK
                  </span>
                </div>

                <h3 className="mt-5 sm:mt-6 text-xl sm:text-3xl font-bold text-white leading-tight">
                  Help us make <span className="text-[#EF8354]">FinSage</span> better for you
                </h3>

                <p className="mt-4 sm:mt-6 text-xs sm:text-base text-white/70 max-w-xl">
                  Your opinion matters. Tell us what you like, what feels
                  confusing, or what features you want to see next in FinSage.
                </p>
              ...
              <div className="mt-6 sm:mt-8 text-[10px] sm:text-xs text-white/60">
                FinSage • Track • Plan • Grow
              </div>
            </div>

            {/* RIGHT FORM */}
            <div className="rounded-2xl sm:rounded-3xl bg-white/10 border border-white/10 backdrop-blur-xl p-4 sm:p-6">
              <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">

                {/* Name */}
                <div>
                  <label className="text-[10px] sm:text-xs text-white/70 flex items-center gap-2">
                    <User className="h-4 w-4 text-white/60" />
                    Name
                  </label>
                  <input
                    ...
                    onChange={handleChange}
                    required
                    className="mt-2 w-full rounded-xl bg-white/10 border border-white/10 px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-white placeholder:text-white/40 outline-none focus:border-[#EF8354]/70"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    required
                    className="mt-2 w-full resize-none rounded-xl bg-white/10 border border-white/10 px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-white placeholder:text-white/40 outline-none focus:border-[#EF8354]/70"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full rounded-xl bg-[#EF8354] py-3 sm:py-4 text-xs sm:text-sm font-semibold text-white shadow-sm hover:opacity-90 transition disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {loading && <Loader2 className="h-4 w-4 animate-spin" />}
                  {loading ? "Sending..." : "Send Feedback"}
                </button>
                ...
                {error && (
                  <div className="rounded-2xl bg-red-500/10 border border-red-500/30 px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-white/90">
                    {error}
                  </div>
                )}

                {submitted && (
                  <div className="rounded-2xl bg-white/10 border border-white/10 px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-white/80">
                    Thank you! Your feedback was received.
                  </div>
                )}

                <p className="text-[10px] sm:text-[11px] text-white/50 text-center mt-2">
                  We use your feedback to improve your FinSage experience.
                </p>
              </form>
            </div>
